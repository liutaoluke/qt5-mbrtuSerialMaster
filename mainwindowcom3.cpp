...
MainWindowCom3::MainWindowCom3(QTimer *p_timer, OS *p_os, QWidget *parent)
    : QMainWindow(parent)
    , ui(new Ui::MainWindowCom3) {
    ui->setupUi(this);
    qDebug() << "constructor - MainWindowCom3::MainWindowCom3(QTimer *p_timer, OS *p_os, QWidget *parent)";

    connect(ui->pushButtonBack, &QPushButton::clicked, this, [=]() {
        qDebug() << "****** MainWindowCom3 before emit sg_backToMainWindow() ******";
        emit sg_backToMainWindow();
        qDebug() << "****** MainWindowCom3 after emit sg_backToMainWindow() ******";
    });
...
MainWindowCom3::~MainWindowCom3() {
    delete ui;
    qDebug() << "destructor - MainWindowCom3::~MainWindowCom3()";
}
