...
MainWindowCom3::MainWindowCom3(QWidget *parent)
    : QMainWindow(parent)
    , ui(new Ui::MainWindowCom3) {
    ui->setupUi(this);
    qInfo() << "constructor - MainWindowCom3::MainWindowCom3(QWidget *parent)";

    connect(ui->pushButtonBack, &QPushButton::clicked, this, [=]() {
        qInfo() << "****** MainWindowCom3 before emit sg_backToMainWindow() ******";
        emit sg_backToMainWindow();
        qInfo() << "****** MainWindowCom3 after emit sg_backToMainWindow() ******";
    });

    // QTimer *p_timer = &os[3].m_qtimerIntvl.intvl_01sec;
...
MainWindowCom3::~MainWindowCom3() {
    qCritical() << "destructor - MainWindowCom3::~MainWindowCom3()";
    delete ui;
}
